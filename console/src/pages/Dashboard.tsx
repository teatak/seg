...
import { RefreshCw, Database, MessageSquare, Activity, TrendingUp } from 'lucide-react';
...
export default function Dashboard() {
    ...
    const statCards = [
        { title: '基础词库', key: 'base_words', icon: Database, gradient: 'from-blue-500/20 to-blue-600/10', iconColor: 'text-blue-600 dark:text-blue-400' },
        { title: '用户词库', key: 'user_words', icon: TrendingUp, gradient: 'from-emerald-500/20 to-emerald-600/10', iconColor: 'text-emerald-600 dark:text-emerald-400' },
        { title: '暂存词库', key: 'staging_words', icon: Database, gradient: 'from-amber-500/20 to-amber-600/10', iconColor: 'text-amber-600 dark:text-amber-400' },
        { title: '用户反馈', key: 'feedback_total', icon: MessageSquare, gradient: 'from-violet-500/20 to-violet-600/10', iconColor: 'text-violet-600 dark:text-violet-400' },
        { title: '请求记录', key: 'requests_total', icon: Activity, gradient: 'from-rose-500/20 to-rose-600/10', iconColor: 'text-rose-600 dark:text-rose-400' },
    ];

    const getValue = (key: string) => {
        if (!stats) return 0;
        if (key === 'base_words') return stats.dictionary.base_words;
        if (key === 'user_words') return stats.dictionary.user_words;
        if (key === 'staging_words') return stats.dictionary.staging_words;
        if (key === 'feedback_total') return stats.feedback.total;
        if (key === 'requests_total') return stats.requests?.total || 0;
        return 0;
    };
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">系统概览与统计数据</p>
                </div>
